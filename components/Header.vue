<template>
  <header
    :class="[
      'bg-white shadow-sm z-50 sticky top-0',
...
onMounted(() => {
  if (isHomePage.value) window.addEventListener('scroll', handleScroll, { passive: true })
})
...
<style scoped>
.header-scroll-anim { transition: transform 0.35s cubic-bezier(0.4, 0, 0.2, 1); will-change: transform; }
...
.nav-link-active { color: #1a73e8 !important; font-weight: 600 !important; }
...
.nav-link-mobile:hover { color: #1a73e8; }
.nav-link-mobile:hover::after { width: calc(100% - 40px); }
.nav-link-mobile-active { color: #1a73e8 !important; font-weight: 600 !important; }
.nav-link-mobile-active::after { width: calc(100% - 40px); }

.mobile-menu-enter-active { transition: max-height 0.35s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.25s ease; }
.mobile-menu-leave-active { transition: max-height 0.3s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.2s ease; }
...
.hamburger-line { transition: transform 0.3s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.2s ease; transform-origin: center; }
.hamburger-btn.is-active .hamburger-line.top { transform: translateY(6px) rotate(45deg); }
.hamburger-btn.is-active .hamburger-line.mid { opacity: 0; }
.hamburger-btn.is-active .hamburger-line.bot { transform: translateY(-6px) rotate(-45deg); }
</style>
