    ...
    <section class="py-16 bg-slate-50 min-h-[60vh]">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div v-else>
          ...
          <div class="product-grid-fade" :key="currentPage + activeUnit">
...
useHead({
  ...
  meta: [
    ...
    { property: 'og:title", content: "产品中心 - 河南玺铭电力科技有限公司' },
    { property: 'og:description', content: '高压清洗设备、智能控制系统、安全防护装备及多系列专业工业清洗产品。' },
    { name: 'keywords', content: '高压清洗设备,智能控制系统,安全防护装备,工业清洗产品,带电清洗设备,清洗药剂,检测仪器' },
  ]
})
...
onMounted(() => {
  ...
  if (unitTabsRef.value) {
    const observer = new IntersectionObserver(
      ([entry]) => {
        scrolledPast.value = !entry.isIntersecting
      },
      { threshold: 0 }
    )
    observer.observe(unitTabsRef.value)
